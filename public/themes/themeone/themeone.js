...
function modalOpen(e){
    if(e.target.tagName == 'IMG'){
        ...
        if(idIndex > -1){
            // id of the currently selected bookmark list id is always first 3
            // letters of bookmarks section title (auto generated by startpage)
            let listID = e.target.parentNode.href.substring(idIndex + 1, idIndex + 4);
...
function colorImage(e){
    if(e.target.tagName == 'IMG'){
        if(e.target.src.indexOf('-colour') == -1){
            e.target.setAttribute('src', e.target.src.slice(0, e.target.src.indexOf('.png')) + '-colour.png');
        }
    }
}
function bwImage(e){
    if(e.target.tagName == 'IMG'){
        if(e.target.src.indexOf('-colour') > -1){
            e.target.setAttribute('src', e.target.src.slice(0, e.target.src.indexOf('-colour')) + '.png');
        }
    }
}
/*
